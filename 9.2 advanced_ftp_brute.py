...
def main():
    ...
    parser.add_argument('--host', type=str, required=True,
                        help='FTP server host or IP.')
    parser.add_argument('--port', type=int, default=21,
                        help='FTP server port. Default is 21.')
    parser.add_argument('-t', '--threads', type=int,
                        default=30, help='Number of threads to use.')
    parser.add_argument('-u', '--user', type=str, help='A single username.')
    parser.add_argument('-U', '--userlist', type=str,
                        help='Path to the usernames list.')
    parser.add_argument('-w', '--wordlist', type=str,
                        help='Path to the passwords list.')
    parser.add_argument('-g', '--generate', action='store_true',
                        help='Generate passwords on the fly.')
    parser.add_argument('--min_length', type=int,
                        help='Minimum length for password generation.', default=1)
    parser.add_argument('--max_length', type=int,
                        help='Maximum length for password generation.', default=4)
    ...
    if args.wordlist:
        ...
        passwords = generate_passwords(
            args.min_length, args.max_length, args.chars)
    else:
    ...
    if args.wordlist:
        ...
        print(f"[+] Passwords to try: {len(list(passwords))}")
    else:
        print(f"[+] Usernames to try: {len(users)}")
